...
import { Calendar as CalendarIcon, Filter, Layers, Search, ListTodo } from 'lucide-react';
...
export default function AdminPanel() {
    ...
    useEffect(() => {
        axios.get('http://localhost:5000/api/appointments')
            ...
            .catch(err => {
                ...
                setError("Failed to load appointments. Please ensure backend is running.");
                setLoading(false);
            });
    }, []);
    ...
    const filteredAppointments = appointments.filter(app => 
        ...
        (app.dentistName && app.dentistName.toLowerCase().includes(searchTerm.toLowerCase()))
    );
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 flex items-center gap-3">
                        <ListTodo className="h-8 w-8 text-blue-600" /> Administrative Dashboard
                    </h1>
                                    ...
                                    <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                ...
                                                <div className="ml-4">
                                                    <div className="text-sm font-medium text-slate-900">{app.patientName}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-slate-900 font-medium">{app.dentistName || 'Unknown'}</div>
                                            <div className="text-xs text-slate-500 truncate max-w-[150px]">{app.clinicName || 'Unknown Clinic'}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
